...
def test_validate_editorial_provenance_allows_missing_source_title_when_source_id_is_consistent():
    p = _p(
        id="p-hobbit",
        book="The Hobbit",
        factual_claims={"smaug_location": "erebor"},
...
def test_corpus_sources_divergence_artifact_includes_inconsistent_count():
    from book_graph_analyzer.cli import main
    ...
    with runner.isolated_filesystem():
        ...
        with open("data/output/hobbit_editorial_divergence_report.json", encoding="utf-8") as f:
            report = json.load(f)
        assert "inconsistent_count" in report["provenance_validation"]
